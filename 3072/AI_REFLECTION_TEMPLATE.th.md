...
| Item | Answer |
|---|---|
| OJ problem number/title | 3072 |
| OJ submission ID, if submitted | 572413 |
| OJ status | Pass |
...
| I used AI as a coach, reviewer, debugger, or test-case helper, not as a full-answer generator. | Yes | ใช้เพื่อเป็นโค้ชในการเขียนโค้ด และเป็นผู้ช่วยในการตรวจสอบ |
...
ฉันถาม Ai ให้ช่วยในการเพิ่มจำนวนสระว่า เมื่อมีการรับข้อความเข้ามา นำมาเข้าเงื่อนไขเพื่อเช็คว่ามีตัวอะไรบ้าง และมีกี่ตัว สมมติว่ารับค่าข้อความ Hello.World จะเห็นว่ามี e 1 ตัว และ O 2 ตัว เลยให้ Ai ช่วยไกด์ในการที่จะเพิ่มจำนวนของสระแต่ละตัวที่เจอในข้อความ
...
ช่วยให้ฉันสังเกตุในการตรวจเช็คว่าสระที่มีในข้อความนั้นอยู่ตำแหน่งที่เท่าไหร่ของ list ที่สร้างด้วย .index() จากนั้นเมื่อรู้ตำแหน่งแล้วก็เขียน count[list] += 1 เพื่อที่เมื่อรู้ตำแหน่งแล้ว ให้ตำแหน่งนั้นมีค่าเพิ่มขึ้นตามจำนวนของสระที่มีในข้อความ
...
ฉันได้ตรวจสอบที่เหลือเอง ะพร้อมทั้งการแสดงคำตอบที่แสดงเฉพาะสระที่มีในข้อความ
...
ได้เรียนรู้ในการใช้ .index() เพื่อเช็คตำแหน่งของสระที่มีในข้อความว่ามันอยู่ที่ตำแหน่งไหนของ list ที่สร้างขึ้นเพื่อรับจำนวนของสระ 
